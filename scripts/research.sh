#!/bin/zsh
# The research sweep. This was the daily job while the project was trying to trade; those questions are settled
# (see ASSUMPTIONS.md — the bonding curve cannot pay, 0 of 19,412 positions ever reached 5x) and the daily job is now
# scripts/daily.sh, the product pipeline. Kept because the measurements are how the settled conclusions were reached
# and are worth re-running when a claim is challenged. Not scheduled; run it by hand.
#   zsh scripts/research.sh
cd "$(dirname "$0")/.." || exit 1
export PATH="/usr/local/bin:/opt/homebrew/bin:$PATH"
day=$(date +%F)
out="data/reports/research-$day.txt"
{
