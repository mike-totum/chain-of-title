...
const DOCS: Record<string, Record<string, Doc>> = {
  tokens: {
    ...
    curve_buyers: { kind: "live", desc: "Distinct wallets that bought on the bonding curve before it graduated. NULL means unknown, which never certifies as clean. Unknown has two causes and they are different: we did not watch the launch, or we watched it on a venue whose trade events carry no wallet. In neither case is the count 0. This is the column an absence hurts most - zero outside buyers is the strongest thing this archive says against a launch - so it is NULL rather than counted from a table the venue was never going to have rows in." },
    snap30_buyers: { kind: "live", desc: "Distinct buyers within the first 30 seconds. Meaningful only for an observed launch. Recorded only where the venue's trade events name the wallet that traded; NULL where they do not, which is not zero and not a clean result. See venue." },
